...
function noiseList(n) {
  return function(done) {
    done(null, n <= 0 ? [] : /* _ */ Array(n + 1).join(0)
      .split(0)
      .map(function(_, i) {
        return i
      }))
...
function read(filename, done) {
  return isString(filename) ? fs.readFile(filename, {
    encoding: 'utf-8'
  }, done) : /* otherwise */ done(null, String(filename))
}
...
function makeLists(dir, noiseFactor, done) {
  parallel([noiseList(noiseFactor), readFiles(dir)], done)
}
